...
def consume_events():
    ...
    channel.exchange_declare(exchange=EXCHANGE_SHIPMENT, exchange_type='topic', durable=True)
    queue = channel.queue_declare(queue='order_service_queue', durable=True).method.queue
    channel.queue_bind(queue=queue, exchange=EXCHANGE_PAYMENT, routing_key='payment.completed')
    channel.queue_bind(queue=queue, exchange=EXCHANGE_PAYMENT, routing_key='payment.failed')
    channel.queue_bind(queue=queue, exchange=EXCHANGE_SHIPMENT, routing_key='shipment.created')
    ...
    logger.info('Order service consuming events...')
