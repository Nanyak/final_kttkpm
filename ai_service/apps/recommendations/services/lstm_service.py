...
Loads the active next-item model (rnn / lstm / bilstm) and returns
next-item probability scores for a given user's recent behaviour sequence.

The active model is controlled by the ACTIVE_MODEL Django setting
(env var ACTIVE_MODEL, default: 'lstm').
"""
...
def _load_model():
    ...
    model_type = getattr(settings, 'ACTIVE_MODEL', 'lstm').lower()
...
def get_lstm_scores(user_id: int, top_k: int = 20) -> Dict[int, float]:
    """
    ...
    with torch.no_grad():
        if model_type == 'bert4rec':
            # Replace the last real token with [MASK]; predict from that position
            x[0, -1] = model.mask_token
            output = model(x)          # (1, seq_len, vocab+2)
            probs = torch.softmax(output[0, -1], dim=0)
        else:
            output = model(x)          # (1, vocab+1)
