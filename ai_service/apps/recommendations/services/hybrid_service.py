...
from .lstm_service  import get_lstm_scores
...
def recommend(
    ...
    w1/w2/w3: weight overrides (default: settings.LSTM/GRAPH/RAG_WEIGHT)
    """
    top_n = top_n or settings.TOP_N
    w1    = w1    if w1 is not None else settings.LSTM_WEIGHT
    ...
    lstm_scores  = get_lstm_scores(user_id,  top_k=top_n * 3)
    ...
    all_pids = set(lstm_scores) | set(graph_scores) | set(rag_scores)

    ranked = []
    for pid in all_pids:
        ls = lstm_scores.get(pid, 0.0)
        gs = graph_scores.get(pid, 0.0)
        rs = rag_scores.get(pid, 0.0)
        final = w1 * ls + w2 * gs + w3 * rs
        ranked.append({
            'product_id':  pid,
