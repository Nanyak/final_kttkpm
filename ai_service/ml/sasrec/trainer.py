"""
Train NextItemSASRec and persist weights + preprocessor to disk.

SASRec uses causal self-attention (no label leakage) so it is trained
like any other next-item model with standard cross-entropy loss.
"""
import json
import pickle
...
from .model import NextItemSASRec
...
def train(
    ...
    Saves sasrec.pt, preprocessor.pkl, meta.json to model_dir.
    """
    ...
    optimiser = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.9, 0.98))
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append(avg)
        if verbose:
            print(f'[SASRec] Epoch {epoch:>3}/{epochs}  loss={avg:.4f}')

    torch.save(model.state_dict(), model_dir / 'sasrec.pt')
    with open(model_dir / 'preprocessor.pkl', 'wb') as f:
        pickle.dump(proc, f)
    meta = {
        'model_type': 'sasrec', 'vocab_size': proc.vocab_size,
    ...
    if verbose:
        print(f'[SASRec] Saved to {model_dir}')
...
def load(model_dir: Path):
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.load_state_dict(torch.load(model_dir / 'sasrec.pt', map_location=device))
    model.eval()
