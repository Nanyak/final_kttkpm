"""
Train NextItemRNN and persist weights + preprocessor to disk.
"""
import json
import pickle
...
def train(
    ...
    Saves rnn.pt, preprocessor.pkl, meta.json to model_dir.
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append(avg)
        if verbose:
            print(f'[RNN]  Epoch {epoch:>3}/{epochs}  loss={avg:.4f}')
    ...
    if verbose:
        print(f'[RNN]  Saved to {model_dir}')
    return model, proc, meta


def load(model_dir: Path):
    """Load trained RNN + preprocessor from disk."""
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.load_state_dict(torch.load(model_dir / 'rnn.pt', map_location=device))
    model.eval()
