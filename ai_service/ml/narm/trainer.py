"""
Train NextItemNARM and persist weights + preprocessor to disk.

NARM uses a GRU encoder with a local attention gate; the training loop
is identical to other sequence models — only the model class differs.
"""
import json
import pickle
...
from .model import NextItemNARM
...
def train(
    ...
    Saves narm.pt, preprocessor.pkl, meta.json to model_dir.
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append(avg)
        if verbose:
            print(f'[NARM]   Epoch {epoch:>3}/{epochs}  loss={avg:.4f}')

    torch.save(model.state_dict(), model_dir / 'narm.pt')
    with open(model_dir / 'preprocessor.pkl', 'wb') as f:
        pickle.dump(proc, f)
    meta = {
        'model_type': 'narm', 'vocab_size': proc.vocab_size,
    ...
    if verbose:
        print(f'[NARM]   Saved to {model_dir}')
...
def load(model_dir: Path):
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.load_state_dict(torch.load(model_dir / 'narm.pt', map_location=device))
    model.eval()
