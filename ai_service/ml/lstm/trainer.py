"""
Train NextItemLSTM and persist weights + preprocessor to disk.
"""
import json
import pickle
...
from .model import NextItemLSTM
...
def train(
    ...
    Saves lstm.pt, preprocessor.pkl, meta.json to model_dir.
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append(avg)
        if verbose:
            print(f'[LSTM] Epoch {epoch:>3}/{epochs}  loss={avg:.4f}')
    ...
    if verbose:
        print(f'[LSTM] Saved to {model_dir}')
    return model, proc, meta


def load(model_dir: Path):
    """Load trained LSTM + preprocessor from disk."""
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.load_state_dict(torch.load(model_dir / 'lstm.pt', map_location=device))
    model.eval()
