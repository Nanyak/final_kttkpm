"""
Train NextItemGRU and persist weights + preprocessor to disk.
"""
import json
import pickle
...
def train(
    ...
    Saves gru.pt, preprocessor.pkl, meta.json to model_dir.
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append(avg)
        if verbose:
            print(f'[GRU]    Epoch {epoch:>3}/{epochs}  loss={avg:.4f}')

    torch.save(model.state_dict(), model_dir / 'gru.pt')
    with open(model_dir / 'preprocessor.pkl', 'wb') as f:
        pickle.dump(proc, f)
    meta = {
        'model_type': 'gru', 'vocab_size': proc.vocab_size,
    ...
    if verbose:
        print(f'[GRU]    Saved to {model_dir}')
...
def load(model_dir: Path):
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.load_state_dict(torch.load(model_dir / 'gru.pt', map_location=device))
    model.eval()
